 ...
 */
export function registerAllBrowserTools(registry) {
    // ═══════════════════════════════════════════
    // NAVIGATION TOOLS
    // ═══════════════════════════════════════════
    registry.register({
    ...
    registry.register({
        ...
        handler: async (args, ctx) => {
            ...
            if (!wc)
                return { error: 'No active tab' };
            const mods = args.modifiers ? args.modifiers.split(',').map((m) => m.trim()) : [];
    ...
    registry.register({
        ...
        handler: async (args, ctx) => {
            ...
            return { success: true, cookies: cookies.map((c) => ({ name: c.name, value: c.value.substring(0, 30), domain: c.domain })) };
        }
    });
    registry.register({
        name: 'set_cookie',
            ...
            try {
                ...
                for (const step of steps) {
                    if (step.action === 'click' && step.element_id) {
                        ...
                        wc.sendInputEvent({ type: 'keyDown', keyCode: step.key });
                        wc.sendInputEvent({ type: 'keyUp', keyCode: step.key });
    ...
    registry.register({
        ...
        handler: async (args, ctx) => {
            ...
            while (Date.now() - startTime < timeout) {
                ...
                if (found)
                    return { success: true, found: true, waited: Date.now() - startTime };
    ...
    registry.register({
        ...
        handler: async (args, ctx) => {
            ...
            wc.sendInputEvent({ type: 'mouseDown', x: args.from_x, y: args.from_y, button: 'left', clickCount: 1 });
            ...
            wc.sendInputEvent({ type: 'mouseUp', x: args.to_x, y: args.to_y, button: 'left', clickCount: 1 });
            ...
            for (let i = 0; i < steps.length; i++) {
                ...
                try {
                    switch (step.action) {
                        ...
                        case 'press_key': {
                            const keyMap = { 'Enter': '\r', 'Tab': '\t', 'Escape': '\u001b', 'Backspace': '\b' };
                                ...
                                for (const subStep of subSteps) {
                                    ...
                                    try {
                                        ...
                                        if (subAction === 'click_text') {
                                            ...
                                            wc.sendInputEvent({ type: 'keyDown', keyCode: subStep.key });
                                            wc.sendInputEvent({ type: 'char', keyCode: subStep.key });
                                            wc.sendInputEvent({ type: 'keyUp', keyCode: subStep.key });
                                            loopCompleted++;
                                        }
        ...
        handler: async (args, ctx) => {
            try {
                ...
                if (httpResult.success && httpResult.results.length > 0) {
                    httpResult._fallback = 'fast_search';
                    return httpResult;
                }
            ...
            if (!result.success || result.results.length === 0) {
                try {
                    ...
                    if (shadowResult.success && shadowResult.results.length > 0) {
                        shadowResult._fallback = 'shadow_search';
                        return shadowResult;
                    }
                }
                catch (_) { }
        ...
        handler: async (args, ctx) => {
            ...
            try {
                ...
                if (shadowResult.success && shadowResult.content.length > (readResult.content?.length || 0)) {
                    shadowResult._fallback = 'shadow_read_page';
                    return shadowResult;
                }
            }
            catch (_) { }
            ...
            try {
                const fp = await getFootprint();
                fp.recordPageVisit(args.url, readResult.title || '', 0);
            }
            catch (_) { }
    ...
    registry.register({
        ...
        handler: async (args, ctx) => {
            ...
            if (type === 'history' || type === 'all')
                result.recentHistory = fp.getRecentHistory(count);
            if (type === 'frequent' || type === 'all')
                result.frequentSites = fp.getFrequentSites(count);
            if (type === 'searches' || type === 'all')
                result.recentSearches = fp.getRecentSearches(count);
            if (type === 'videos' || type === 'all')
                result.unfinishedVideos = fp.getVideoResume();
            if (type === 'all') {
...
function getSOMFinderScript(elementId) {
    return `
...
async function performSOMAction(ctx, elementId, action) {
    const wc = ctx.webContents;
    if (!wc)
        return { error: 'No active tab' };
    ...
    try {
        ...
        if (action === 'click') {
            ...
            try {
                await wc.executeJavaScript(fallbackScript);
            }
            catch (_) { }
            ...
            if (cx > 0 && cy > 0) {
                ...
                wc.sendInputEvent({ type: 'mouseUp', x: cx, y: cy, button: 'left', clickCount: 1 });
                await new Promise(r => setTimeout(r, 50));
                wc.sendInputEvent({ type: 'mouseDown', x: cx, y: cy, button: 'left', clickCount: 2 });
                wc.sendInputEvent({ type: 'mouseUp', x: cx, y: cy, button: 'left', clickCount: 2 });
            }
