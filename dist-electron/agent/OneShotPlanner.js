...
export class OneShotPlanner {
     ...
     */
    constructor(apiKeyOrRotator, modelName = 'gemini-2.5-flash') {
    ...
    async generateTaskJSON(params) {
        ...
        const fewShotContext = relevantExamples.map(ex => `EXAMPLE:\nGoal: "${ex.goal}"\nDomain: ${ex.domain}\nOutput:\n${JSON.stringify(ex.json)}`).join('\n\n');
...
Return ONLY valid JSON. Cover EVERY step from current state to goal completion.`;
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            ...
            try {
                ...
                if (!parsed.task_intent)
                    parsed.task_intent = goal;
                if (!parsed.task_id)
                    parsed.task_id = `t_${Date.now().toString(36)}`;
                ...
                for (let i = 0; i < parsed.steps.length; i++) {
                    ...
                    if (!step.on_failure) {
                        step.on_failure = { retry_count: 1, retry_strategy: 'same_action', then: 'skip', escalate_after_retries: 'skip' };
                    }
                    if (!step.glass_box) {
                ...
                if (this.keyRotator && keyIndex >= 0)
                    this.keyRotator.recordSuccess(keyIndex);
                ...
                const isRateLimit = error.message && (error.message.includes('429 Resource exhausted') ||
     ...
     */
    selectRelevantExamples(domain, goal, count) {
        const goalLower = goal.toLowerCase();
        const scored = FEW_SHOT_EXAMPLES.map(ex => {
            let score = 0;
            // Domain match
            if (domain.includes(ex.domain) || ex.domain.includes(domain))
                score += 5;
            // Goal word overlap
    ...
    static shouldUseOneShot(goal) {
        ...
        if (hasOneShotSignal && !hasReactiveSignal)
            return true;
...
export class CausalWebModel {
    ...
    recordTransition(fromHash, toHash, action, success) {
        ...
        if (existing) {
            ...
            this.transitions.push({ from: fromHash, to: toHash, action, success, count: 1 });
        }
    }
    predictOutcome(currentHash, action) {
    ...
    getStats() {
        ...
        for (const t of this.transitions)
            acts.set(t.action, (acts.get(t.action) || 0) + t.count);
