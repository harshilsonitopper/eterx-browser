...
export class ImmuneMemory {
    ...
    learnFromFailure(context) {
        ...
        if (antibody) {
            ...
            const existing = this.antibodies.find(a => a.trigger_pattern === antibody.trigger_pattern && a.domain === antibody.domain);
            if (existing) {
    ...
    scan(context) {
        ...
        for (const ab of this.antibodies) {
            ...
            if (!domainMatch)
                continue;
            // Pattern match
            const patternMatch = ab.trigger_pattern.includes(context.tool) ||
...
export class NetworkOracle {
     ...
     */
    getContextSummary(maxEntries = 3) {
     ...
     */
    hasSuccessfulCall(urlPattern, method) {
        return this.responses.some(r => r.url.includes(urlPattern) &&
