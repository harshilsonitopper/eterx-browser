...
import { CausalWebModel } from './OneShotPlanner.js';
import { SkillVault, SkillComposer, AutonomousWorkLoop } from './SkillFabric.js';
...
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function buildSystemPrompt(memory, currentDomain, footprintContext) {
...
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function classifyComplexity(goal) {
...
export class NextGenAgent {
    ...
    async executeObjective(task, options) {
        if (this.isRunning) {
            this.stop();
        ...
        try {
            ...
            if (this.activeWebContentsId) {
                ...
                if (cached) {
                    ...
                    const executor = new TaskExecutor(this.activeWebContentsId, (msg, type) => this.log(msg, type));
                ...
                try {
                    ...
                    if (['email', 'gmail', 'inbox', 'mail'].some(w => taskLower.includes(w))) {
                        ...
                        try {
                            ...
                            if (recent.length > 0)
                                delegateContext += 'Recent pages: ' + recent.map(p => p.title).join(', ') + '. ';
                            if (loggedIn.length > 0)
                                delegateContext += 'Logged in: ' + loggedIn.join(', ') + '.';
                        }
                        catch (_) { }
    ...
    // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    async reactLoop(goal, maxTurns, zeroClick = false) {
        ...
        const createSession = async (modelOverride, history) => {
            ...
            const model = client.genAI.getGenerativeModel({
                ...
                tools: [{ functionDeclarations: this.registry.toGeminiFunctionDeclarations() }],
            });
            return history
        ...
        while (this.state.turnCount < maxTurns && this.isRunning) {
            ...
            try {
                ...
                this.log('Tab destroyed mid-turn', 'error');
                continue;
            }
            ...
            if (urlChanged && turn > 1) {
                ...
                try {
                    await this.networkOracle.startCapture(wc);
                }
                catch (_) { }
            }
            ...
            if (turn === 1 || urlChanged) {
                try {
                    ...
                    if (parsed.signals.hasCaptcha) {
                        ...
                        const domain = this.getCurrentDomain();
                        if (domain)
                            this.memory.recordCaptchaDetection(domain, 'recaptcha_v2');
                        try {
            ...
            try {
                ...
                if (nextUrls.length > 0)
                    this.agiPrefetch.prefetchUrls(nextUrls);
            }
            catch (_) { }
            ...
            try {
                await wc.executeJavaScript(SOM_CLEANUP_SCRIPT);
            }
            catch (_) { }
            if (!screenshotBase64) {
                await new Promise(r => setTimeout(r, 500));
            ...
            const domInfo = somElements.slice(0, this.agiSpeed.getSOMLimit()).map((el) => {
                ...
                if (el.aria)
                    line += ` aria="${el.aria}"`;
                if (el.href)
                    line += ` href=${el.href.substring(0, 50)}`;
            ...
            if (stuckEscapeLevel >= 1) {
                ...
                if (stuckEscapeLevel >= 2)
                    warnings += ` Use evaluate_js, navigate to a different URL, or call task_failed.`;
                if (stuckEscapeLevel >= 3)
                    warnings += ` LAST CHANCE â€” if this doesn't work, call task_failed.`;
                warnings += '\n';
            }
...
This executes ALL steps in ONE call (microseconds) instead of wasting more turns.\n`;
            ...
            try {
                await this.networkOracle.harvest(wc);
            }
            catch (_) { }
            ...
            let pageTypeHint = '';
            try {
                const formDetect = somElements.filter((el) => ['input', 'textarea', 'select'].includes(el.tag));
                if (formDetect.length > 0) {
                    const inputs = formDetect.filter((el) => el.tag === 'input' || el.tag === 'textarea');
                    const selects = formDetect.filter((el) => el.tag === 'select');
                    const emptyFields = inputs.filter((el) => !el.val);
                    const requiredFields = formDetect.filter((el) => el.req);
                    const hasSubmit = somElements.some((el) => (el.tag === 'button' || el.type === 'submit') &&
                    ...
                    if (emptyFields.length > 0) {
                        const fieldList = emptyFields.slice(0, 8).map((f) => `  → ${f.name || f.ph || f.aria || `${f.tag}[${f.type}]`}${f.req ? ' [REQUIRED]' : ''} @(${f.x},${f.y})`).join('\n');
                        pageTypeHint += `\nEMPTY FIELDS TO FILL:\n${fieldList}`;
                        pageTypeHint += `\n💡 TIP: Use batch_fill_form({fields:{${emptyFields.slice(0, 5).map((f) => `"${f.name || f.ph || f.aria || f.type}":"value"`).join(',')}}) to fill all at once.`;
                    }
                }
            }
            catch (_) { }
            ...
            let routerHints = '';
            try {
                routerHints = this.agiRouter.getRoutingHints();
            }
            catch (_) { }
            let navHints = '';
            try {
...
Analyze the screenshot and take the next best action toward the goal.`;
                ...
                if (allFunctionCalls.length > 0) {
                    ...
                    for (const functionCall of allFunctionCalls) {
                        ...
                        if (this.window && !this.window.isDestroyed()) {
                            ...
                            if (args.url)
                                detail = args.url;
                            else if (args.text && name.includes('type'))
                                detail = `"${(args.text || '').substring(0, 50)}"`;
                            else if (args.query)
                                detail = args.query;
                            else if (args.target)
                                detail = args.target;
                            else if (args.element_id !== undefined)
                                detail = `element #${args.element_id}`;
                            else if (args.task)
                                detail = args.task;
                                ...
                                detail = `${Array.isArray(args.steps) ? args.steps.length : '?'} steps`;
                        ...
                        for (let _attempt = 0; _attempt < 2; _attempt++) {
                            toolResult = await Promise.race([
                                ...
                                new Promise(r => setTimeout(() => r({ success: false, error: 'Tool timed out after 60s' }), 60000))
                            ]);
                            // Retry on transient errors (timeout, network)
                            if (toolResult.error && _attempt === 0 && (toolResult.error.includes('timed out') ||
                        ...
                        if (toolResult.error) {
                            ...
                            if (alts[name])
                                toolResult._suggestion = alts[name];
                        ...
                        if (!this._recentActionSignatures)
                            this._recentActionSignatures = [];
                        this._recentActionSignatures.push(actionSig);
                        if (this._recentActionSignatures.length > 5)
                            this._recentActionSignatures.shift();
                        const repeats = this._recentActionSignatures.filter((s) => s === actionSig).length;
                        if (repeats >= 3) {
                        ...
                        if (this.registry.isTerminal(name)) {
                            if (name === 'task_complete') {
                                ...
                                const interactionToolsUsed = this.state.toolsUsed.filter(t => ['click', 'type_text', 'click_by_text', 'fill_form_smart', 'select_option', 'press_key', 'click_and_wait', 'find_and_click', 'batch_fill_form', 'auto_type_and_submit', 'execute_js_sequence', 'read_page_content', 'evaluate_js', 'navigate', 'shadow_search', 'shadow_read_page', 'shadow_read_emails', 'shadow_execute', 'shadow_agent_task', 'shadow_research', 'shadow_fill_and_submit', 'execute_action_plan', 'scroll_to_text', 'click_xy', 'fast_search', 'fast_read', 'parallel_search', 'parallel_read', 'get_user_history'].includes(t)).length;
    ...
    createToolContext() {
        ...
        return {
            ...
            executeTool: (name, args) => this.registry.execute(name, args, this.createToolContext()),
            humanDelay: (min, max) => this.humanDelay(min, max),
            memory: {
                get: (key) => this.memory.getVariable(key),
                set: (key, val) => this.memory.setVariable(key, val),
            },
        };
    }
    ...
    async humanDelay(min = 200, max = 600) {
