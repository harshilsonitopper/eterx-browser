...
class ShadowBrowser {
    ...
    async init() {
        ...
        try {
            ...
            this.window.webContents.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36');
            this.ready = true;
        }
        catch (e) {
    ...
    async loadURL(url, timeoutMs = 15000) {
        await this.init();
        if (!this.window || this.window.isDestroyed())
            return false;
...
export class ShadowAgent {
    ...
    async search(query, maxResults = 8) {
        ...
        try {
            ...
            if (!loaded)
                return { success: false, query, results: [], error: 'Failed to load Google' };
    ...
    async readPage(url, options = {}) {
        ...
        try {
            ...
            if (!loaded)
                return { success: false, url, title: '', content: '', error: 'Failed to load page' };
    ...
    async research(query, depth = 3) {
        try {
            ...
            const pageContents = rawResults.filter(Boolean);
    ...
    async quickAnswer(question) {
        ...
        if (featured && featured.snippet)
            return { answer: featured.snippet, source: 'Google Featured Snippet', success: true };
        const top = searchResult.results[0];
        if (top)
            return { answer: top.snippet, source: top.url, success: true };
        return { answer: '', source: '', success: false };
    }
    /**
     ...
     */
    async executeAction(url, script) {
        try {
            this.isWorking = true;
            const fullUrl = url.startsWith('http') ? url : `https://${url}`;
            const loaded = await this.browser.loadURL(fullUrl);
            if (!loaded)
                return { success: false, url, result: null, error: 'Failed to load page' };
     ...
     */
    async fillAndSubmit(url, fields, submitSelector) {
        try {
            this.isWorking = true;
            const fullUrl = url.startsWith('http') ? url : `https://${url}`;
            const loaded = await this.browser.loadURL(fullUrl);
            if (!loaded)
                return { success: false, url, filled: 0, submitted: false, error: 'Failed to load page' };
     ...
     */
    async readEmails(maxEmails = 10) {
        try {
            this.isWorking = true;
            const loaded = await this.browser.loadURL('https://mail.google.com/mail/u/0/#inbox');
            if (!loaded)
                return { success: false, emails: [], error: 'Failed to load Gmail' };
    ...
    get busy() { return this.isWorking; }
...
export class ShadowPool {
    ...
    async acquireBrowser() {
        ...
        for (let i = 0; i < this.browsers.length; i++) {
            if (!this.inUse.has(i)) {
                ...
                if (!this.browsers[i] || this.browsers[i].window?.isDestroyed?.()) {
                    try {
    ...
    async withBrowser(fn) {
        ...
        try {
            ...
            return await Promise.race([
                fn(browser),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Pool operation timed out')), 45000))
            ]);
        }
        finally {
    ...
    async parallelSearch(queries, maxResults = 6) {
        const start = Date.now();
        const promises = queries.map(query => this.withBrowser(async (browser) => {
    ...
    async parallelRead(urls, maxLength = 3000) {
        const start = Date.now();
        const promises = urls.map(url => this.withBrowser(async (browser) => {
            // Check cache first
            const cached = this.cache.get(url);
            if (cached)
                return { url, title: cached.title, content: cached.content };
            const loaded = await browser.loadURL(url);
            if (!loaded)
                return { url, title: '', content: '', error: 'Failed to load' };
    ...
    async parallelExecute(tasks) {
        const start = Date.now();
        const promises = tasks.map(task => this.withBrowser(async (browser) => {
            const loaded = await browser.loadURL(task.url);
            if (!loaded)
                return { url: task.url, result: null, error: 'Failed to load' };
    ...
    get size() { return this.browsers.length; }
    get active() { return this.inUse.size; }
}
// ═════════════════════════════════════════════════
// SINGLETONS
// ═════════════════════════════════════════════════
let _shadowInstance = null;
let _subAgentInstance = null;
let _poolInstance = null;
export function getShadowAgent() {
    if (!_shadowInstance) {
        _shadowInstance = new ShadowAgent();
    }
    return _shadowInstance;
}
export function getShadowSubAgent(apiKey) {
    if (!_subAgentInstance) {
        _subAgentInstance = new ShadowSubAgent(getShadowAgent(), apiKey);
    }
    return _subAgentInstance;
}
export function getShadowPool() {
    if (!_poolInstance) {
        _poolInstance = new ShadowPool();
    }
    return _poolInstance;
}
export function destroyShadowAgent() {
    if (_shadowInstance) {
