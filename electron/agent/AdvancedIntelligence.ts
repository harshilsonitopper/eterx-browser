...
import { TaskJSON, TaskStep } from './SiteFingerprint.js';

// ═══════════════════════════════════════════════════════════════════════
// ADVERSARIAL PRE-MORTEM ENGINE
// From BeyondHuman System 08
// "Imagine this task ALREADY FAILED. What went wrong?"
// ═══════════════════════════════════════════════════════════════════════

export interface PreMortemResult {
    risk_score: number;          // 0-100: overall risk
...
export class AdversarialPreMortem {
     ...
     */
    static analyze(task: TaskJSON, knownFailures?: Map<string, string[]>): PreMortemResult {
...
// From BeyondHuman System 12
// Pre-execute likely next steps while current step completes
// ═══════════════════════════════════════════════════════════════════════

export interface SpeculativeResult {
    step_id: string;
    pre_resolved: boolean;       // Was the element pre-found?
    cached_coords?: { x: number; y: number };
    pre_checked_state?: boolean;
...
// ═══════════════════════════════════════════════════════════════════════
// PROMPT GENOME — Evolutionary Prompt Optimization
// From BeyondHuman System 10
// Tracks prompt variants and evolves based on success metrics
// ═══════════════════════════════════════════════════════════════════════

export interface PromptGene {
    id: string;
    variant: string;            // The prompt text
    category: string;           // 'system' | 'action_tip' | 'warning' | 'site_hint'
    fitness: number;            // 0-1, success rate when this gene is active
    generation: number;
    mutations: number;
    parent_id?: string;
    created_at: number;
...
export class PromptGenome {
    private genes: Map<string, PromptGene> = new Map();
    private activeGenes: Set<string> = new Set();

    /**
     * Register a prompt gene
     */
    addGene(variant: string, category: string): string {
     ...
     */
    updateFitness(geneId: string, success: boolean): void {
     ...
     */
    mutate(geneId: string, newVariant: string): string {
     ...
     */
    evolve(): { survived: number; culled: number } {
        const byCategory = new Map<string, PromptGene[]>();
     ...
     */
    getBestGenes(): PromptGene[] {
        const best = new Map<string, PromptGene>();
        for (const gene of this.genes.values()) {
    ...
    getStats(): { totalGenes: number; avgFitness: number; categories: string[] } {
        let totalFit = 0;
        const cats = new Set<string>();
        for (const g of this.genes.values()) { totalFit += g.fitness; cats.add(g.category); }
        return { totalGenes: this.genes.size, avgFitness: this.genes.size > 0 ? totalFit / this.genes.size : 0, categories: [...cats] };
    }
}

// ═══════════════════════════════════════════════════════════════════════
// GLASS BOX SAFETY — Human-readable action preview + risk gating
// From OneShotJSONBrain Safety Layer
// ═══════════════════════════════════════════════════════════════════════
...
export class GlassBoxSafety {
     ...
     */
    evaluate(step: TaskStep, taskIntent: string): SafetyDecision {
     ...
     */
    private describeAction(step: TaskStep): string {
     ...
     */
    previewPlan(task: TaskJSON): string[] {
