...
import { TaskJSON, SkillDefinition, SuccessCriteria } from './SiteFingerprint.js';
...
export class SkillVault {
    ...
    private triggerIndex: Map<string, string[]> = new Map(); // trigger_word → skill_ids
     ...
     */
    findSkills(intent: string, domain?: string, maxResults: number = 5): SkillDefinition[] {
        ...
        for (const [id, skill] of this.skills) {
            ...
            // Domain bonus
            if (domain && skill.domain === domain) score += 3;
            ...
            if (skill.performance.success_rate > 0.8) score += 2;
            if (skill.performance.execution_count > 10) score += 1;

            if (score > 0) candidateScores.set(id, score);
        }
     ...
     */
    createFromExecution(taskJSON: TaskJSON, executionMetrics: {
        duration_ms: number;
        success: boolean;
     ...
     */
    updatePerformance(skillId: string, success: boolean, durationMs: number): void {
     ...
     */
    getStats(): { totalSkills: number; byCategory: Record<string, number>; byDomain: Record<string, number>; avgSuccessRate: number } {
    ...
    private intentToName(intent: string): string {
        return intent.split(/\s+/).slice(0, 5).map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ');
    }

    private inferCategory(task: TaskJSON): string {
        const intent = task.task_intent.toLowerCase();
        if (intent.match(/email|send|compose|mail/)) return 'communication';
        if (intent.match(/form|survey|questionnaire/)) return 'data_entry';
        if (intent.match(/search|find|lookup|browse/)) return 'research';
        if (intent.match(/create|make|build|add|new/)) return 'creation';
        if (intent.match(/edit|update|change|modify/)) return 'modification';
        if (intent.match(/delete|remove|cancel/)) return 'deletion';
        if (intent.match(/buy|purchase|order|checkout/)) return 'commerce';
        if (intent.match(/post|publish|share|tweet/)) return 'social';
        if (intent.match(/download|upload|export|import/)) return 'file_transfer';
        if (intent.match(/schedule|book|reserve|calendar/)) return 'scheduling';
        if (intent.match(/login|register|signup|account/)) return 'authentication';
        return 'general';
    }

    private generateTriggers(intent: string): string[] {
    ...
    private extractInputs(task: TaskJSON): Record<string, { type: string; description: string; default?: any }> {
    ...
    private inferTags(task: TaskJSON): string[] {
        const tags: string[] = [task.target_domain, task.risk_level];
        const actionTypes = new Set(task.steps.map(s => s.action.type));
        for (const t of actionTypes) tags.push(t);
        return tags;
    }
}

// ═══════════════════════════════════════════════════════════════════════
// SKILL COMPOSER — Chain multiple skills into complex workflows
// ═══════════════════════════════════════════════════════════════════════
...
export interface ComposedWorkflow {
    ...
    skill_chain: Array<{
        skill_id: string;
        input_mapping: Record<string, string>; // Maps output of previous skill to input of this one
...
export class SkillComposer {
    private vault: SkillVault;

    constructor(vault: SkillVault) {
        this.vault = vault;
    }
    ...
    compose(goal: string, domain: string): ComposedWorkflow | null {
        ...
        for (const intent of subIntents) {
            ...
            if (matches.length === 0) return null; // Can't compose if any sub-skill is missing
            ...
            if (riskOrder.indexOf(skill.glass_box.risk_level) > riskOrder.indexOf(maxRisk)) {
                maxRisk = skill.glass_box.risk_level as any;
            }
        }
    ...
    private decomposeGoal(goal: string): string[] {
        ...
        if (andParts.length > 1 && andParts.every(p => p.trim().length > 5)) return andParts;
...
// ═══════════════════════════════════════════════════════════════════════
// AUTONOMOUS WORK LOOP — Background self-directed execution
// ═══════════════════════════════════════════════════════════════════════
...
export interface WorkItem {
    ...
    scheduled_at?: number;     // Unix timestamp for scheduled execution
    repeat_interval_ms?: number; // For recurring tasks
    skill_id?: string;         // Pre-assigned skill
    created_at: number;
    started_at?: number;
    completed_at?: number;
    result?: any;
...
export class AutonomousWorkLoop {
    ...
    private currentItem: WorkItem | null = null;
    private maxConcurrent: number = 1;
    private completedHistory: WorkItem[] = [];
    private maxHistory: number = 100;

    /**
     * Add a work item to the queue
     */
    enqueue(item: Omit<WorkItem, 'id' | 'status' | 'created_at'>): string {
     ...
     */
    scheduleRecurring(goal: string, domain: string, intervalMs: number, priority: WorkItem['priority'] = 'low'): string {
        return this.enqueue({
    ...
    dequeue(): WorkItem | null {
        if (this.currentItem) return null; // Already running something

        const now = Date.now();
        const idx = this.queue.findIndex(item =>
            item.status === 'queued' && (!item.scheduled_at || item.scheduled_at <= now)
        );
    ...
    getStats(): {
        ...
        currentItem: WorkItem | null; nextItem: WorkItem | null;
    } {
     ...
     */
    getHistory(limit: number = 10): WorkItem[] {
...
export class SkillOptimizer {
     ...
     */
    static analyze(skill: SkillDefinition): {
        suggestions: string[];
        canRemoveSteps: string[];
        canMergeSteps: string[][];
        estimatedSpeedup: number;
    } {
        const suggestions: string[] = [];
        const canRemoveSteps: string[] = [];
        const canMergeSteps: string[][] = [];
        let speedup = 1.0;

        const plan = skill.execution_plan;
        if (!plan || !plan.steps) return { suggestions, canRemoveSteps, canMergeSteps, estimatedSpeedup: 1.0 };
        ...
        for (const step of plan.steps) {
            if (step.action.type === 'type') { consecutiveTypes++; }
            else {
     ...
     */
    static optimize(skill: SkillDefinition): SkillDefinition {
        const analysis = this.analyze(skill);
        if (analysis.suggestions.length === 0) return skill;

        const optimized = JSON.parse(JSON.stringify(skill)) as SkillDefinition;

        // Remove skippable steps
        if (analysis.canRemoveSteps.length > 0) {
            optimized.execution_plan.steps = optimized.execution_plan.steps.filter(
                s => !analysis.canRemoveSteps.includes(s.step_id)
            );
            // Re-chain steps
    ...
    private static bumpVersion(version: string): string {
