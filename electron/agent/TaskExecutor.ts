...
import { TaskJSON, TaskStep, ExpectedState, TargetStrategy } from './SiteFingerprint.js';
...
export interface StepResult {
    step_id: string;
    status: 'success' | 'failed' | 'skipped' | 'human_handoff';
    duration_ms: number;
    retries_used: number;
...
export interface ExecutionResult {
    task_id: string;
    status: 'complete' | 'partial' | 'failed' | 'human_handoff';
...
class ElementResolver {
    private wcId: number;
    constructor(webContentsId: number) { this.wcId = webContentsId; }
    private get wc() { return webContents.fromId(this.wcId); }

    async resolve(target: TargetStrategy): Promise<{ strategy: string; coords?: { x: number; y: number } } | null> {
        const wc = this.wc;
     ...
     */
    private extractCellRef(target: string): string | null {
     ...
     */
    private async sheetsNavigateToCell(cellRef: string): Promise<boolean> {
        const wc = this.wc;
        ...
        try {
            ...
            wc.sendInputEvent({ type: 'mouseDown', x: 60, y: 140, button: 'left', clickCount: 1 } as any);
            await new Promise(r => setTimeout(r, 30));
            wc.sendInputEvent({ type: 'mouseUp', x: 60, y: 140, button: 'left', clickCount: 1 } as any);
    ...
    private async trySemantic(sel: string): Promise<{ coords?: { x: number; y: number } } | null> {
        const wc = this.wc;
        if (!wc || wc.isDestroyed()) return null;
        if (sel.startsWith('url:')) return { coords: undefined };
        try {
    ...
    private async tryCSS(sel: string): Promise<{ coords?: { x: number; y: number } } | null> {
        const wc = this.wc;
        if (!wc || wc.isDestroyed()) return null;
        try {
    ...
    private async tryCoord(sel: string): Promise<{ coords: { x: number; y: number } } | null> {
        if (sel.startsWith('xy:')) {
            const [x, y] = sel.replace('xy:', '').split(',').map(Number);
            if (!isNaN(x) && !isNaN(y)) return { coords: { x, y } };
        }
        return null;
    }
}
...
class StateVerifier {
    ...
    async verify(expected: ExpectedState, timeoutMs: number = 5000): Promise<boolean> {
        const wc = this.wc;
        if (!wc || wc.isDestroyed()) return false;
        ...
        while (Date.now() - start < timeoutMs) {
            ...
            if (expected.url_contains) {
                if (!wc.getURL().toLowerCase().includes(expected.url_contains.toLowerCase())) ok = false;
            }
...
class ActionDispatcher {
    ...
    async dispatch(step: TaskStep, resolver: ElementResolver): Promise<{ success: boolean; error?: string; output?: any }> {
        const wc = this.wc;
        if (!wc || wc.isDestroyed()) return { success: false, error: 'WebContents destroyed' };
        const { action } = step;
        const t = action.type;
        ...
        try {
            ...
            if (t === 'wait_for_navigation') {
                ...
                while (Date.now() - start < timeout) {
                    if (wc.getURL() !== orig) return { success: true };
            ...
            if (t === 'click' || t === 'click_text') {
                ...
                if (!res?.coords) return { success: false, error: `Element not found: ${ action.target.primary }` };
            ...
            if (t === 'click_nth') {
                ...
                if (!coords) return { success: false, error: `Nth element ${ n + 1 } not found` };
                await this.click(coords.x, coords.y);
                return { success: true };
            }
            if (t === 'double_click_text') {
                const res = await resolver.resolve(action.target);
                if (!res?.coords) return { success: false, error: `Element not found` };
                wc.sendInputEvent({ type: 'mouseDown', x: res.coords.x, y: res.coords.y, button: 'left', clickCount: 2 } as any);
                await this.sleep(30);
                wc.sendInputEvent({ type: 'mouseUp', x: res.coords.x, y: res.coords.y, button: 'left', clickCount: 2 } as any);
                return { success: true };
            }
            if (t === 'right_click') {
                const res = await resolver.resolve(action.target);
                if (!res?.coords) return { success: false, error: `Element not found` };
                wc.sendInputEvent({ type: 'mouseDown', x: res.coords.x, y: res.coords.y, button: 'right', clickCount: 1 } as any);
                await this.sleep(30);
                wc.sendInputEvent({ type: 'mouseUp', x: res.coords.x, y: res.coords.y, button: 'right', clickCount: 1 } as any);
                return { success: true };
            }
            if (t === 'hover_text') {
                const res = await resolver.resolve(action.target);
                if (!res?.coords) return { success: false, error: `Element not found` };
                wc.sendInputEvent({ type: 'mouseMove', x: res.coords.x, y: res.coords.y } as any);
            ...
            if (t === 'select_all_type') {
                const res = await resolver.resolve(action.target);
                if (res?.coords) await this.click(res.coords.x, res.coords.y);
                await this.sleep(50);
                await wc.executeJavaScript(`document.execCommand('selectAll')`);
                wc.insertText(action.value || '');
                return { success: true };
            ...
            if (t === 'clear_field') {
                ...
                await this.sleep(50);
                await wc.executeJavaScript(`document.execCommand('selectAll')`);
                wc.sendInputEvent({ type: 'keyDown', keyCode: 'Delete' } as any);
                wc.sendInputEvent({ type: 'keyUp', keyCode: 'Delete' } as any);
                return { success: true };
            }
            if (t === 'append_text') {
                const res = await resolver.resolve(action.target);
            ...
            if (t === 'select_option') {
                ...
                if (action.value) {
                    ...
                    if (!ok) return { success: false, error: `Option "${ action.value }" not found` };
                }
                return { success: true };
            }
            if (t === 'check_box') {
            ...
            if (t === 'submit_form') {
                const res = await resolver.resolve(action.target);
                if (res?.coords) { await this.click(res.coords.x, res.coords.y); return { success: true }; }
            ...
            if (t === 'press_key') {
                ...
                wc.sendInputEvent({ type: 'keyDown', keyCode: mapped } as any);
                wc.sendInputEvent({ type: 'keyUp', keyCode: mapped } as any);
                return { success: true };
            }
            if (t === 'hotkey' || t === 'key_combo') {
                ...
                for (const k of combo) {
                    ...
                    if (kl === 'ctrl' || kl === 'control') mods.control = true;
                    else if (kl === 'shift') mods.shift = true;
                    else if (kl === 'alt') mods.alt = true;
                    else if (kl === 'meta' || kl === 'cmd') mods.meta = true;
                    else keys.push(k.trim());
                }
                for (const key of keys) {
                    wc.sendInputEvent({ type: 'keyDown', keyCode: key, ...mods } as any);
                    wc.sendInputEvent({ type: 'keyUp', keyCode: key, ...mods } as any);
                }
                return { success: true };
            }
            ...
            if (t === 'wait_for_url') {
                ...
                const start = Date.now();
                while (Date.now() - start < timeout) {
                    if (wc.getURL().toLowerCase().includes(pattern.toLowerCase())) return { success: true };
                    await this.sleep(300);
                }
            ...
            // ═══ TABLE & BULK ═══
            if (t === 'fill_table') {
                const cells = JSON.parse(action.value || '[]') as Array<{ row: number; col: number; text: string }>;
            ...
            if (t === 'drag_drop') {
                ...
                const dstTarget: TargetStrategy = { primary: action.value || '' };
                const dst = await resolver.resolve(dstTarget);
                if (!src?.coords || !dst?.coords) return { success: false, error: 'Drag source or target not found' };
                wc.sendInputEvent({ type: 'mouseDown', x: src.coords.x, y: src.coords.y, button: 'left', clickCount: 1 } as any);
                await this.sleep(100);
                wc.sendInputEvent({ type: 'mouseMove', x: dst.coords.x, y: dst.coords.y } as any);
                await this.sleep(100);
                wc.sendInputEvent({ type: 'mouseUp', x: dst.coords.x, y: dst.coords.y, button: 'left', clickCount: 1 } as any);
                return { success: true };
            }
            if (t === 'multi_select') {
                const items = JSON.parse(action.value || '[]') as string[];
                for (const item of items) {
                    const target: TargetStrategy = { primary: item };
                    const res = await resolver.resolve(target);
                    if (res?.coords) {
                        // Ctrl+Click for multi-select
                        wc.sendInputEvent({ type: 'mouseDown', x: res.coords.x, y: res.coords.y, button: 'left', clickCount: 1, modifiers: ['control'] } as any);
                        await this.sleep(30);
                        wc.sendInputEvent({ type: 'mouseUp', x: res.coords.x, y: res.coords.y, button: 'left', clickCount: 1, modifiers: ['control'] } as any);
                        await this.sleep(100);
                    }
                }
                return { success: true };
            ...
            // ═══ DIALOGS ═══
            if (t === 'dismiss_dialog') {
                wc.sendInputEvent({ type: 'keyDown', keyCode: 'Escape' } as any);
                wc.sendInputEvent({ type: 'keyUp', keyCode: 'Escape' } as any);
                await this.sleep(300);
            ...
            if (t === 'triple_click') {
                const res = await resolver.resolve(action.target);
                if (!res?.coords) return { success: false, error: `Element not found` };
                wc.sendInputEvent({ type: 'mouseDown', x: res.coords.x, y: res.coords.y, button: 'left', clickCount: 3 } as any);
                await this.sleep(30);
                wc.sendInputEvent({ type: 'mouseUp', x: res.coords.x, y: res.coords.y, button: 'left', clickCount: 3 } as any);
                return { success: true };
            }
            if (t === 'human_handoff') {
...
export class TaskExecutor {
    ...
    constructor(webContentsId: number, logFn?: (msg: string, type?: string) => void) {
    ...
    async execute(task: TaskJSON): Promise<ExecutionResult> {
        const startTime = Date.now();
        const stepResults: StepResult[] = [];
        let currentStepId: string | undefined = task.steps[0]?.step_id;
        const stepIndex = new Map<string, TaskStep>();
        for (const step of task.steps) stepIndex.set(step.step_id, step);
        let stepsCompleted = 0;

        this.logFn(`📋 Executing: "${ task.task_intent }" (${ task.steps.length } steps)`);

        while (currentStepId && stepIndex.has(currentStepId)) {
            const step: TaskStep = stepIndex.get(currentStepId)!;
            const stepStart = Date.now();
            let retriesUsed = 0, success = false, error: string | undefined, fallbackUsed: string | undefined, output: any;

            this.logFn(`  ▸ ${ step.step_id }: ${ step.label }`);

            const maxRetries = step.on_failure?.retry_count || 1;
            for (let attempt = 0; attempt <= maxRetries; attempt++) {
                if (attempt > 0) { retriesUsed++; await new Promise(r => setTimeout(r, 500)); }
                ...
                if (result.success) {
                    if (step.expected_state && Object.keys(step.expected_state).length > 0) {
                        if (await this.verifier.verify(step.expected_state, step.action.timing?.timeout_ms || 5000)) { success = true; break; }
                        else error = 'State verification failed';
            ...
            if (!success && step.on_failure?.then?.startsWith('execute_fallback:')) {
                ...
                if (fb) {
                    ...
                    for (const a of fb.actions) {
                        const fbStep: TaskStep = { step_id: `fb`, label: 'fallback', risk: 'low', action: { type: a.type as any, target: { primary: a.target || '' }, value: a.direction || String(a.amount || ''), timing: { wait_for: 'dom_stable', timeout_ms: 3000, poll_interval_ms: 200 } }, expected_state: {}, on_success: '', on_failure: { retry_count: 0, retry_strategy: 'same_action', then: 'skip', escalate_after_retries: 'skip' }, glass_box: { show_in_ui: false, label_for_human: '', pause_before_execute: false, highlight_element: false } };
            ...
            if (success) {
                ...
                const next: string = step.on_success.replace('proceed_to:', '');
                currentStepId = next === 'done' ? undefined : next;
            } else {
                this.logFn(`    ❌ ${ step.label }: ${ error }`, 'error');
                const esc = step.on_failure?.escalate_after_retries;
                if (esc === 'abort') break;
                if (esc === 'human_handoff') { stepResults[stepResults.length - 1].status = 'human_handoff'; break; }
                currentStepId = step.on_success.replace('proceed_to:', '');
                if (currentStepId === 'done') currentStepId = undefined;
            }

            if (stepResults.length > task.steps.length * 3) break; // Safety
        }
...
export class TemplateLibrary {
    private templates: Map<string, { task: TaskJSON; intent: string; successCount: number; lastUsed: number }> = new Map();

    store(task: TaskJSON): void {
    ...
    findMatch(intent: string, domain: string): TaskJSON | null {
        const key = this.genKey(intent, domain);
        const exact = this.templates.get(key);
        if (exact) { exact.lastUsed = Date.now(); return exact.task; }
        let best: { task: TaskJSON; score: number } | null = null;
        ...
        for (const [, e] of this.templates) {
            ...
            if (score > 0.6 && (!best || score > best.score)) best = { task: e.task, score };
        }
        return best?.task || null;
    }

    getStats(): { totalTemplates: number; totalExecutions: number } {
        let total = 0;
        for (const [, e] of this.templates) total += e.successCount;
        return { totalTemplates: this.templates.size, totalExecutions: total };
    }

    private genKey(intent: string, domain: string): string {
