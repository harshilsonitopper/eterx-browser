...
function extractCaptionsUrl(html: string): string | null {
    ...
    try {
        ...
        const manual = tracks.find((t: any) => !t.kind && t.languageCode === 'en');
        const autoEn = tracks.find((t: any) => t.kind === 'asr' && t.languageCode === 'en');
...
function parseJson3Transcript(json: any): TranscriptSegment[] {
 ...
 */
export function createTimestampedSummary(segments: TranscriptSegment[], chunkDuration: number = 60): {
    chunks: { timestamp: string; startSeconds: number; text: string }[];
    totalDuration: string;
} {
    const chunks: { timestamp: string; startSeconds: number; text: string }[] = [];
