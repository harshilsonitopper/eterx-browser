...
import { app, BrowserWindow, shell, ipcMain, webContents, Menu, nativeImage, clipboard, powerSaveBlocker, protocol } from 'electron'
...
import { BrowserAgentService } from './BrowserAgentService.js'; import { DeepAgent } from './DeepAgent.js'; // Legacy (deprecated)
...
if (!fs.existsSync(ETERX_TEMP_DIR)) fs.mkdirSync(ETERX_TEMP_DIR, { recursive: true });
...
let permissionStore: Record<string, Record<string, boolean>> = {}; // { domain: { permission: boolean } }
...
let powerSaveId: number | null = null; // ⚡ Prevent OS from throttling when minimized
...
// Google OAuth (Loopback method for desktop apps)
...
ipcMain.handle('update-site-permission', (_, { origin, permission, allowed }) => {
  if (!permissionStore[origin]) permissionStore[origin] = {};
...
// =============================================================================
// GEMINI LIVE NATIVE AUDIO (SDK via Main Process)
// Enhanced with Auto Key Rotation & Health Tracking
// =============================================================================

// Key Health Tracking Interface
interface KeyHealth {
  lastError: number;
  errorCount: number;
  exhausted: boolean;
...
class GeminiKeyManager {
  ...
  private readonly COOLDOWN_MS = 60000; // 1 minute cooldown for exhausted keys
  private readonly MAX_ERRORS = 10; // High tolerance for transient errors
  ...
  markError(index: number, errorType: 'quota' | 'rate_limit' | 'network' | 'unknown') {
  ...
  getStatus(): { total: number; healthy: number; currentIndex: number; keys: { index: number; healthy: boolean; errorCount: number }[] } {
...
let browserAgent: BrowserAgentService; // Keep legacy as fallback
let nextGenAgent: NextGenAgent;
try {
  console.log('[Main] 🤖 Initializing NextGenAgent (v2)...');
  const keysToUse = API_KEYS.length > 0 ? API_KEYS : [process.env.VITE_API_KEY || ''];
  if (!keysToUse[0]) console.error('[Main] ❌ NO API KEY FOUND! Agent will not work.');
...
let geminiCurrentKeyIndex: number = -1;
let geminiEventSender: Electron.WebContents | null = null;
let geminiIsConnecting: boolean = false; // Lock to prevent race conditions
let audioChunkCounter: number = 0; // Debug counter for audio chunks
const toolCallMap = new Map<string, string>(); // Store tool ID -> Name mapping

let activeWebContentsId: number | null = null;
let originalWebContentsId: number | null = null;
let visionLoopInterval: NodeJS.Timeout | null = null;
 ...
 */
async function handleGhostTask(task: string, id: string) {
  if (isGhosting) {
...
let geminiSessionHandle: string | null = null; // Session resumption handle
...
async function captureScreenFast(wc: Electron.WebContents, quality: number = 40): Promise<string | null> {
  try {
    ...
    if (jpegBuffer.length < 500) return null;
...
function startVisionLoop() {
  ...
  visionLoopInterval = setInterval(async () => {
    if (!geminiSession || !activeWebContentsId) return;
...
function sendGeminiStatus(status: 'disconnected' | 'connecting' | 'connected' | 'reconnecting' | 'error', details?: any) {
 ...
 */
async function dispatchToolCall(name: string, args: any, id: string) {
    ...
    if (wc && !wc.isDestroyed()) {
      try {
        ...
        if (name === 'click_coordinates' || name === 'click_and_type') {
          ...
          // Native Click
          wc.sendInputEvent({ type: 'mouseDown', x: px, y: py, button: 'left', clickCount: 1 } as any);
          await new Promise(r => setTimeout(r, 50));
          wc.sendInputEvent({ type: 'mouseUp', x: px, y: py, button: 'left', clickCount: 1 } as any);
          ...
          if (args.amount === 'little') delta = 100;
          if (args.amount === 'full') delta = 800;
...
async function connectGeminiLive(sender: Electron.WebContents) {
  ...
  try {
    ...
    geminiSession = await ai.live.connect({
      ...
      config: {
        ...
        tools: [
          { googleSearch: {} },
          { functionDeclarations: browserControlTools as any }
        ]
      },
      callbacks: {
        onopen: () => {
          ...
          if (activeWebContentsId) startVisionLoop();
        },
        ...
        onerror: (e: any) => {
          ...
          let type: 'quota' | 'network' | 'unknown' = 'unknown';
...
ipcMain.on('gemini:realtime-input', (event, part: { mimeType: string; data: string }) => {
  if (!geminiSession) {
...
ipcMain.on('gemini:client-content', (event, text: string) => {
...
function createWindow() {
  ...
  preconnectDomains.forEach(url => {
    try { win?.webContents.session.preconnect({ url, numSockets: 2 }); } catch (e) { }
  });
  ...
  const disableAllThrottling = () => {
    webContents.getAllWebContents().forEach(wc => {
      if (!wc.isDestroyed()) {
        try { wc.backgroundThrottling = false; } catch (e) { }
      }
    });
  };
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('browser-agent:start', async (event, task, zeroClickMode) => {
    ...
    if (!nextGenAgent) return { error: 'Agent not initialized' };
    if (!activeWebContentsId) return { error: 'No active tab' };

    const mainWin = BrowserWindow.fromWebContents(event.sender);
    if (mainWin) nextGenAgent.setTargetWindow(mainWin);
    nextGenAgent.setActiveWebContentsId(activeWebContentsId);
  ...
  ipcMain.handle('browser-agent:stop', async () => {
    ...
    if (nextGenAgent) nextGenAgent.stop();
    return { status: 'stopped' };
  });

  ipcMain.handle('agent:get-state', async () => {
    if (nextGenAgent) return nextGenAgent.getState();
    return { status: 'idle' };
  });
...
ipcMain.handle('ai:capture-screenshot', async (_, url: string) => {
...
ipcMain.handle('app:open-external-file', async (_, filePath: string) => {
  try {
...
ipcMain.handle('app:save-temp-file', async (_, fileName: string, buffer: ArrayBuffer) => {
  try {
...
app.on('web-contents-created', (event, contents) => {
  ...
  try { contents.backgroundThrottling = false; } catch (e) { }
  ...
  if (contents.getType() === 'webview') {
    const enableZoom = () => {
      try { contents.setVisualZoomLevelLimits(1, 4); } catch (err) { console.error(`[Zoom] Failed for ${ contents.id }:`, err); }
    };
    enableZoom();
  ...
  contents.on('did-attach-webview', (e, webContents) => {
    ...
    try { webContents.setVisualZoomLevelLimits(1, 4); } catch (e) { }
...
ipcMain.handle('ai:tts', async (_event: any, text: string, voiceName: string = 'Kore') => {
  ...
  try {
    ...
    if (!keyResult) return { success: false, error: 'All API keys exhausted' };
...
ipcMain.handle('file:export', async (_event: any, { content, type, filename }: { content: string, type: 'pdf' | 'markdown' | 'code' | 'text', filename: string }) => {
  try {
    const fs = require('fs');
    ...
    const extMap: Record<string, string> = { pdf: '.pdf', markdown: '.md', code: path.extname(filename) || '.js', text: '.txt' };
    let finalName = filename;
    if (!finalName.includes('.')) finalName += extMap[type] || '.txt';
      ...
      function mdToHtml(md: string): string {
        ...
        h = h.replace(/((\|[^\n]+\|\n)+)/g, (tableBlock: string) => {
          ...
          rows.forEach((row: string, i: number) => {
            ...
            t += `<tr${ rc }>` + cells.map((c: string) => `<${ tag }>${ c.trim() }</${ tag }>`).join('') + '</tr>';
          });
          return t + '</table>';
        });
...
ipcMain.handle('terminal:run', async (_event: any, { command, timeout = 30000 }: { command: string, timeout?: number }) => {
  try {
    ...
    return new Promise((resolve) => {
      const child = exec(command, { timeout, maxBuffer: 1024 * 1024 }, (error: any, stdout: string, stderr: string) => {
...
ipcMain.handle('file:convert', async (_event: any, { inputPath, outputFormat }: { inputPath: string, outputFormat: string }) => {
  try {
    const { exec } = require('child_process');
    const path = require('path');
...
if (!fs.existsSync(ETERX_WORKSPACE)) fs.mkdirSync(ETERX_WORKSPACE, { recursive: true });
...
try {
  ...
  try { execSync('python3 --version', { stdio: 'pipe' }); pythonPath = 'python3'; }
  catch { pythonPath = 'python'; }
} catch (_) { }

console.log(`[Terminal] 🐍 Python path: ${ pythonPath }, Workspace: ${ ETERX_WORKSPACE }`);

ipcMain.handle('terminal:execute', async (_event: any, command: string, timeoutMs: number = 60000) => {
  const { exec } = require('child_process');
...
ipcMain.handle('terminal:list-files', async () => {
  try {
    const files = fs.readdirSync(ETERX_WORKSPACE).map((f: string) => {
...
ipcMain.handle('terminal:read-file', async (_event: any, filename: string) => {
  try {
...
ipcMain.handle('terminal:pip-install', async (_event: any, packages: string) => {
  const { exec } = require('child_process');
  return new Promise((resolve) => {
...
try {
  ...
  ipc.on('eterx:video-state', (_event: any, data: any) => {
    import('./agent/UserFootprint.js').then(mod => {
