import { BrowserWindow, webContents, ipcMain, app } from 'electron';
...
export class DeepAgent {
    ...
    constructor(apiKey: string, modelType: '2.5-flash' | '2.5-pro' | '2.5-flash-lite' = '2.5-flash') {
...
Coordinate System: 1024x768. X=0,Y=0 is top-left.`,
    ...
    public setActiveWebContents(id: number) {
    ...
    private async agentLoop(goal: string) {
        let turn = 0;
        ...
        while (turn < this.config.maxTurns) {
            ...
            try {
                ...
                if (call) {
                    ...
                    if (this.window) this.window.webContents.send('gemini:thought', text);
    ...
    private async executeTool(name: string, args: any): Promise<any> {
        if (!this.activeWebContentsId) return { error: "No active tab" };
        const wc = webContents.fromId(this.activeWebContentsId);
        if (!wc || wc.isDestroyed()) return { error: "Tab destroyed" };
    ...
    private async clickAt(wc: Electron.WebContents, x: number, y: number) {
