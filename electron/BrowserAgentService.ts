...
export class BrowserAgentService {
    ...
    public setTargetWindow(win: BrowserWindow) {
        this.window = win;
    }

    public setActiveWebContentsId(id: number) {
        this.activeWebContentsId = id;
    }

    private log(message: string, type: 'info' | 'error' | 'action' = 'info') {
    ...
    public async executeObjective(task: string) {
        if (this.isRunning) {
            this.stop();
    ...
    private async captureBrowserViewport(): Promise<string | null> {
        if (!this.activeWebContentsId) return null;
    ...
    private async autonomousLoop(goal: string) {
        let currentModelName = 'gemini-2.5-flash';
        let keyRotationCount = 0;

        const createSession = (modelName: string, history?: any[]) => {
...
CRITICAL: Never guess an element_id. If you don't see a red box with a number, use evaluate_js or scroll.`,
                tools: [
                    { functionDeclarations: BROWSER_TOOLS as any }
                ]
            });
        ...
        while (interactionTurn < MAX_TURNS && this.isRunning) {
            ...
            try {
                ...
                const cleanDom = somElements.map((el: any) => ({ id: el.id, tag: el.tag, text: el.text }));
    ...
    private async orchestrateToolCall(name: string, args: any): Promise<any> {
        if (!this.activeWebContentsId) return { error: "No active browser tab found." };
        const wc = webContents.fromId(this.activeWebContentsId);
        if (!wc || wc.isDestroyed()) return { error: "Browser tab was closed unexpectedly." };
    ...
    private async performSetOfMarkAction(wc: Electron.WebContents, id: number, action: 'click' | 'right_click' | 'hover' | 'focus'): Promise<any> {
